...
<? endif ?>
<form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data" class="promo__form form mb-5">
				...
				<div class="col-12 col-md-6 form-group position-relative">
					...
					?><br /><?
						...
						case "N":
							...
							?>
								<input type="text" id="inp-<?= $propertyID; ?>" class="form-control form__input bg-light rounded-0 border border-secondary text-black fs-20 lh-12 p-3 h-100 skew-btn" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?= $value ?>" <? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) : ?>required<? endif ?>>
								<label for="inp-<?= $propertyID; ?>" class="form-label form__label fs-20 lh-12 text-primary position-absolute top-50">
