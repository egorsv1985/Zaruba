...
<form name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post" enctype="multipart/form-data" class="promo__form form px-sm-5">
	...
	<div class="row gy-2">
					...
					<div class=" form-group position-relative mb-1">
						...
						?>
							...
							case "N":
								...
								?>
									<input type="text" id="inp-<?= $propertyID; ?>" class="form-control form__input bg-light rounded-0 border border-secondary text-black fs-20 lh-12 p-3 h-100" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?= $value ?>" <? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) : ?>required<? endif ?> placeholder="<? if (intval($propertyID) > 0) : ?><?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"] ?><? else : ?><?= !empty($arParams["CUSTOM_TITLE_" . $propertyID]) ? $arParams["CUSTOM_TITLE_" . $propertyID] : GetMessage("IBLOCK_FIELD_" . $propertyID) ?><? endif ?><? if (in_array($propertyID, $arResult["PROPERTY_REQUIRED"])) : ?> *<? endif ?>">
								...
								switch ($type):
									...
									case "radio":
										...
										?>
											...
											<label for="property_<?= $key ?>"><?= $arEnum["VALUE"] ?>
											</label>
										<?
										}
										break;
		...
		<div class="col-12 col-md-12">
			<div class="form-check form-check-inline form__checkbox position-relative me-0 ps-0 mb-4">
				<input type="checkbox" class="form-check-input checkbox__input m-0" value="" name="check" id="check" checked>
				<label class="form-check-label checkbox__label fs-14 lh-12" for="check">
					<span class="checkbox__text">Я согласен на
						<a href="#" class="">обработку персональных данных</a>
					</span>
				</label>
			</div>
			<button type="submit" name="iblock_submit" class="btn fs-20 fw-800 px-3 py-3 btn-primary rounded-0 w-100 border-0 skew-btn" value="<?= GetMessage("IBLOCK_FORM_SUBMIT") ?>">
				<span>ХОЧУ!</span>
			</button>
		</div>
	</div>
